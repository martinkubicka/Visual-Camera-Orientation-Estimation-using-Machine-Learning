...
def normalize(out, truth):
    ...
    out = torch.stack((pitch, yaw), dim=1)
    return out
...
def train(model, dataloader, loss_fn, optimizer, device):
    ...
    for batch in dataloader:        
        ...
        loss_p = loss_fn(normalize(out, labels), torch.zeros(picture_images.size(0), 2).to(device))
        
        diff = torch.abs(out[:, :2] - labels[:, :2])
        correct_predictions = (diff < 0.15).all(dim=1)
        correct_predictions_count += correct_predictions.sum().item()
        total_predictions_count += correct_predictions.size(0)
