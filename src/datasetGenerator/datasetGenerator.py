...
def saveData(panoID, imageID, latitude, longitude, pitch, yaw, roll):
    ...
    with open(os.path.join(actualDirectory, f"dataset/{panoID}.txt"), "w") as file:
        file.write(data)
...
if __name__ == '__main__':
    ...
    while count < numberOfPanoramas:
        try:
            ...
            panorama.save(os.path.join(actualDirectory, "dataset/" + panoId[0] + "_panorama.jpg"), "jpeg")
            saveData(panoId[0], panoId[1], panoId[2], panoId[3], panoId[4], panoId[5], panoId[6])

            print(str(count + 1) + ". panorama saved to dataset/" + panoId[0] + "_panorama.jpg.")
