...
class Perspective:
    def __init__(self, img_name, FOV, YAW, PITCH, ROLL):
    ...
    def GetEquirec(self, height, width):
        ...
        # Changes for YAW, PITCH, and ROLL
        [R1, _] = cv2.Rodrigues(z_axis * np.radians(self.YAW))
        [R2, _] = cv2.Rodrigues(x_axis * np.radians(-self.ROLL))
        [R3, _] = cv2.Rodrigues(y_axis * np.radians(-self.PITCH))
